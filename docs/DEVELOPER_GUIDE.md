...
- Run `pwsh -File tools/Test-ForkSimulation.ps1` to create a disposable branch, open a draft PR from the fork, and exercise **VI Compare (Fork PR)** end-to-end. Pass `-KeepBranch` to retain the scratch branch/PR, or `-DryRun` to preview the steps without mutating the repository.
