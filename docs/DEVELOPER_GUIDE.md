...
- Defaults to LabVIEW 2021 SP1 (32-bit and 64-bit) for packed libraries and LabVIEW 2025 (64-bit) for packaging.
  ...
  - Packaging always routes through the VIPM CLI (`-BuildToolchain vipm-cli`). The switch is retained for forwards-compatibility but currently accepts only `vipm-cli`, and the manifest records the requested toolchain under `packaging.requestedToolchain` for downstream diagnostics.
