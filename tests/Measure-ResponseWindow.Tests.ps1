Describe 'Measure-ResponseWindow script' -Tag 'Unit' {
  ...
  It 'Start then End produces withinMargin = True for close timings' {
    ...
    & $global:__scriptPath -Action Start -Reason 'measure-unit' -ExpectedSeconds 1 -ToleranceSeconds 5 -ResultsDir $resultsDir | Out-Null
    Start-Sleep -Milliseconds 1050
    & $global:__scriptPath -Action End -ResultsDir $resultsDir -ToleranceSeconds 5 -FailOnOutsideMargin:$false | Out-Null
  ...
  It 'FailOnOutsideMargin sets nonzero exit code when outside' {
    ...
    New-Item -ItemType Directory -Path $resultsDir -Force | Out-Null
    & $global:__scriptPath -Action Start -Reason 'measure-unit-2' -ExpectedSeconds 0 -ToleranceSeconds 0 -ResultsDir $resultsDir | Out-Null
    Start-Sleep -Milliseconds 1100
    $exe = (Get-Command pwsh).Source
    $args = @('-NoLogo','-NoProfile','-File', $global:__scriptPath, '-Action','End','-ResultsDir', $resultsDir, '-ToleranceSeconds','0','-FailOnOutsideMargin')
