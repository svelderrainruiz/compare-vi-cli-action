...
- First actions in a session:
  1. `node tools/npm/run-script.mjs priority:sync` to refresh the standing-priority snapshot and router
     ...
     Ensure a GitHub token is available via `GH_TOKEN`/`GITHUB_TOKEN` or by pointing `GH_TOKEN_FILE` at a
     token file (default fallback: `C:\github_token.txt`). Tokens are injected into the container without
     being printed to logs.
