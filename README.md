...
- Artifacts: `vi-compare-manifest` (manifest + summary) and `vi-compare-staging` (numbered `vi-staging-XX.zip` bundles). Each pair’s `compare/` directory now includes `compare-report.html` plus a `compare-report_files/` folder containing the 2025 LVCompare image assets (set `COMPAREVI_REPORT_FORMAT=html-single` if you need the legacy single-file artifact).
