...
if (-not $fixtureResolved) {
  $fixtureResolved = Resolve-PathOptional -Path 'tests/fixtures/icon-editor/ni_icon_editor-1.4.1.948.vip' -BasePath $repoRoot
}
...
if (-not $baselineFixtureResolved) {
  $baselineFixtureResolved = Resolve-PathOptional -Path 'tests/fixtures/icon-editor/ni_icon_editor-1.4.1.794.vip' -BasePath $repoRoot
}

$baselineManifestResolved = Resolve-PathOptional -Path $BaselineManifest -BasePath $repoRoot
if (-not $baselineManifestResolved) {
  $baselineManifestResolved = Resolve-PathOptional -Path 'tests/fixtures/icon-editor/fixture-manifest-1.4.1.794.json' -BasePath $repoRoot
}
