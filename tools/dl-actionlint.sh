...
function usage() {
    ...
    echo >&2
    echo 'If GITHUB_TOKEN or GH_TOKEN is defined, requests will include an authorization'
    echo 'header to dodge rate limits.'
}
...
if [[ "$os" == "windows" ]]; then
    ...
    curl -L "${auth_header[@]}" -o "$tempdir/tmp.zip" "${url}"
    unzip "$tempdir/tmp.zip" actionlint.exe -d "$target_dir"
    rm -r "$tempdir"
    exe="$target_dir/actionlint.exe"
else
    curl -L "${auth_header[@]}" "${url}" | tar xvz -C "$target_dir" actionlint
